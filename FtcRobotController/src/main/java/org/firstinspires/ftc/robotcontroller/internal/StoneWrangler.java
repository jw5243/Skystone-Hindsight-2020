...
public class StoneWrangler {
    ...
    public double getStonePixelX() { return stonePixelX; }
    public double getStonePixelY() { return stonePixelY; }
    public Mat getVisualization() { return visualization; }

    public void analyze(Mat src_) {
        //Mat src = new Mat();
        Core.flip(src_, src_, 0);
        Mat transformed = inversePerspectiveTransform(src_);
        visualization = src_;
        ...
        //reportStonePosition();
        visualization = generateVisualization(src_, transformed.size());
    }
    ...
    public void reportStonePosition(Telemetry telemetry) {
        telemetry.addLine("stone world X: " + stoneWorldX);
        telemetry.addLine("stone world Y: " + stoneWorldY);
        telemetry.addLine("orientation: " + (stoneTheta - Math.PI / 2));
    }
    ...
    private Mat generateVisualization(Mat src, Size flatSize) {
        return addPoseText(
                StoneWranglerUtils.verticalFlip(src));
    ...
    private Mat addPoseText(Mat frame) {
        ...
        StoneWranglerUtils.drawCircle(frame, 280, 124, 4);
        StoneWranglerUtils.drawCircle(frame, 550, 124, 4);
        StoneWranglerUtils.drawCircle(frame, 180, 260, 4);
        StoneWranglerUtils.drawCircle(frame, 710, 260, 4);
        return frame;
    }
}
